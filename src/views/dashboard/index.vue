<template>
  <div class="map_m">
    ...
    <dv-border-box-1 class="park">
      <div class="park_content">
        ...
        <div class="distribution">
          <div>
            <img
              src="@/assets/images/baohui/house-5.png"
              alt=""
              @click="modelShow(0)"
            />
            园区展示
    ...
    <div class="tableData" v-if="cameraList">
      <el-button type="primary" @click="cameraList = false">关闭列表</el-button>
      <el-table :data="cameraData" style="width: 100%" height="850">
        ...
        <el-table-column label="操作">
          <template slot-scope="scope">
            <el-button
              size="mini"
              @click="cameraEdit(scope.$index, scope.row.id)"
              >编辑</el-button
            >
            <el-button size="mini" type="danger" @click="addCamera()"
              >添加</el-button
            >
          </template>
        </el-table-column>
    ...
    >
      ...
      <el-button type="primary" @click="updateCameraXYZ(formCamera.id)"
        >完成</el-button
      >
  ...
  methods: {
    ...
    onMouseclickCamera(event) {
      ...
      if (
        ...
        if (this.flag != intersects[0].object.name) {
          $(".label").css("display", "none");
        }
        $("#" + intersects[0].object.name).toggle();
        this.flag = intersects[0].object.name;
        // 清除播放器列表
        ...
        this.streamedian(intersects[0].object.name, intersects[0].object.Vname);
      } else {
    ...
    streamedian(id, Vname) {
      ...
      // url = 'rtsp://admin:zzxy2009@jtht.tpddns.cn:9683/Streaming/Channels/' + me.state.videoN;
      // me.state.videoN += 100;
